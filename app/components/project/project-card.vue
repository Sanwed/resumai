...
<template>
  <NuxtLink :to="`/projects/${project.id}`">
    <UCard :ui="{
      root: 'w-sm block ring-transparent',
      header: ['border-none p-4 sm:px-4', project.importanceLevel ? 'bg-warning/50' : 'bg-slate-200'],
      body: 'sm:p-4',
      footer: 'sm:p-4',
    }">
      <template #header>
        <div class="flex items-center justify-between">
          ...
          <div class="flex items-center gap-2">
            ...
            <nuxt-link :to="`/projects/${project.id}/settings`" class="size-5 transition-transform hover:scale-110">
      ...
      <div class="flex items-start gap-x-4">
        <div
          class="shrink-0 size-10 rounded-sm bg-primary-50 border border-primary-200 flex justify-center items-center">
        ...
        <p v-if="project.description" class="text-sm text-slate-600">{{ project.description }}</p>
