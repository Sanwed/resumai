...
const { data: projects, pending, refresh: refreshProjects } = await useFetch<Projects[]>('/api/projects');
</script>
...
<template>
  <div v-if="session" class="flex flex-col gap-y-4 md:gap-y-6 p-4 md:px-8 max-w-7xl mx-auto">
    ...
    <div class="flex gap-4">
      <FormCreateProject @on-success="refreshProjects()">
        <template #toggle>
    ...
    <div class="flex flex-col gap-y-2 md:gap-y-4">
      <div class="flex items-center justify-between gap-x-2">
        ...
        <p class="text-sm text-muted">{{ projects?.length ?? 0 }} из {{ MAX_USER_PROJECTS }}</p>
      </div>
